...
type MigInfoProvider interface {
	...
	// RefillAllMigInstances refills MIGs to instances mapping cache
	RefillAllMigInstances() error
...
func (c *cachingMigInfoProvider) GetMigForInstance(instanceRef GceRef) (Mig, error) {
	...
	foundCandidate, err := c.fillCandidateMigInstances(instanceRef)
	if err != nil {
		return nil, err
	}

	mig, found, err = c.getCachedMigForInstance(instanceRef)
	if foundCandidate && !found {
...
func (c *cachingMigInfoProvider) getCachedMigForInstance(instanceRef GceRef) (Mig, bool, error) {
	if migRef, found := c.cache.GetMigForInstance(instanceRef); found {
		...
		return mig, true, nil
	} else if unknown := c.cache.IsMigUnknownForInstance(instanceRef); unknown {
		return nil, true, nil
	}
...
// RefillAllMigInstances refills MIGs to instances mapping cache
func (c *cachingMigInfoProvider) RefillAllMigInstances() error {
...
func (c *cachingMigInfoProvider) fillCandidateMigInstances(instanceRef GceRef) (bool, error) {
	var candidateMig Mig
	...
	if candidateMig != nil {
		return true, c.fillMigInstances(candidateMig.GceRef())
	}
	return false, nil
}
...
func (c *cachingMigInfoProvider) fillMigInstances(migRef GceRef) error {
	...
	if err != nil {
		c.migLister.HandleMigIssue(migRef)
		return err
	}
