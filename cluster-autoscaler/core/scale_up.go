...
func computeScaleUpResourcesLeftLimits(
	...
	if cloudprovider.ContainsGpuResources(resourceLimiter.GetResources()) {
		totalGpus, totalGpusErr = calculateScaleUpGpusTotal(context, processors, nodeGroups, nodeInfos, nodesFromNotAutoscaledGroups)
	}
...
func calculateScaleUpGpusTotal(
	...
	for _, nodeGroup := range nodeGroups {
		...
		if currentSize > 0 {
			gpuType, gpuCount, err := processors.CustomResourceProcessor.GetNodeTargetResource(context,
				gpu.ResourceNvidiaGPU, context.CloudProvider.GPULabel(), nodeInfo.Node(), nodeGroup)
			if err != nil {
	...
	for _, node := range nodesFromNotAutoscaledGroups {
		gpuType, gpuCount, err := processors.CustomResourceProcessor.GetNodeTargetResource(context,
			gpu.ResourceNvidiaGPU, context.CloudProvider.GPULabel(), node, nil)
		if err != nil {
...
func ScaleUp(context *context.AutoscalingContext, processors *ca_processors.AutoscalingProcessors, clusterStateRegistry *clusterstate.ClusterStateRegistry, unschedulablePods []*apiv1.Pod,
	...
	if bestOption != nil && bestOption.NodeCount > 0 {
		...
		for _, info := range scaleUpInfos {
			gpuType := processors.CustomResourceProcessor.GetResourceTypeForMetrics(context,
				gpu.ResourceNvidiaGPU, gpuLabel, availableGPUTypes, nodeInfo.Node(), nil)
			typedErr := executeScaleUp(context, clusterStateRegistry, info, gpuType, now)
			if typedErr != nil {
				return &status.ScaleUpStatus{Result: status.ScaleUpError, CreateNodeGroupResults: createNodeGroupResults}, typedErr
			}
		}
